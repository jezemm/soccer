      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1 flex-1 min-w-0">
          ...
          <div className="leading-tight">
            {(() => { const { club, team } = splitOpponent(game.opponent); return <>{club && <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest truncate">{club}</p>}<p className="text-lg font-black text-emjsc-navy uppercase italic truncate">Vs {team}</p></>; })()}
          </div>
          <a
