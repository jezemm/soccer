...
import { AvatarConfig, AVATAR_OPTIONS, getAvataaarsUrl, getDefaultAvatarConfig, getRandomAvatarConfig, CLUB_LOGO } from '../lib/constants';
import { AvatarImage } from './AvatarImage';
import { LABELS, DISPLAYED_KEYS, formatLabel } from './AvatarEditor';
...
const STEP_LABELS: Record<Step, string> = { welcome: 'Welcome', avatar: 'Avatar', bio: 'Bio', password: 'Password' };
      ...
      <div className="w-full max-w-sm mx-auto flex flex-col h-dvh md:h-auto md:max-h-[92dvh] md:rounded-3xl md:shadow-2xl md:overflow-hidden bg-white">
          ...
          {STEPS.map((step, i) => (
            <React.Fragment key={step}>
              <div className="flex items-center gap-1">
                ...
                <span className={`text-[8px] font-black uppercase tracking-widest hidden sm:block ${i <= stepIndex ? 'text-emjsc-navy' : 'text-slate-300'}`}>
                  {STEP_LABELS[step]}
                </span>
              </div>
              {i < STEPS.length - 1 && (
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4">
          ...
          {currentStep === 'welcome' && (
            <div className="flex flex-col items-center text-center gap-4 h-full justify-center">
              ...
              <div className="space-y-1">
                <h2 className="text-xl font-black text-emjsc-navy uppercase tracking-tight leading-none">Welcome, {playerName}!</h2>
                <p className="text-xs text-slate-500 font-medium leading-relaxed">Let's set up your profile — avatar, bio, and password.</p>
              </div>
              <div className="w-full space-y-1.5 text-left">
                {([
                  ...
                  <div key={label} className="flex items-center gap-2.5 px-3 py-2 bg-slate-50 rounded-xl">
                    <div className="w-6 h-6 rounded-lg bg-emjsc-navy/10 flex items-center justify-center text-emjsc-navy shrink-0">{icon}</div>
                ...
                {visibleKeys.map(key => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-2">
                      <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 w-20 shrink-0">{LABELS[key]}</span>
                      <div className="flex-1 flex items-center gap-1 min-w-0">
                        ...
                        <span className="flex-1 text-center text-[10px] font-bold text-slate-700 truncate px-1">{formatLabel(avatarConfig[key])}</span>
                        <button onClick={() => stepAvatar(key, 1)} className="p-1 rounded-lg bg-slate-100 hover:bg-slate-200 active:scale-95 transition-all shrink-0">
                          <ChevronRight className="w-3.5 h-3.5 text-slate-600" />
                        </button>
                      </div>
                      <span className="text-[9px] text-slate-300 font-bold w-8 text-right shrink-0">{idx + 1}/{opts.length}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {currentStep === 'password' && (
            <div className="flex flex-col gap-4 h-full justify-center">
              ...
              <div className="space-y-3">
                {passwordError && <p className="text-[10px] font-black text-emjsc-red uppercase tracking-wide">{passwordError}</p>}
                <div className="space-y-1.5">
        ...
        <div className="px-5 pt-3 border-t border-slate-100 space-y-2 shrink-0" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}>
          <div className="flex gap-3">
            {stepIndex > 0 && (
              <button onClick={goBack} disabled={saving} className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 active:scale-95 transition-all disabled:opacity-50">
                <ChevronLeft className="w-4 h-4" />
              </button>
            )}
            {currentStep !== 'password' ? (
              <button onClick={goNext} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-emjsc-navy hover:bg-emjsc-red text-white rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all">
                {currentStep === 'welcome'
                  ? <><span>Get Started</span><ArrowRight className="w-3.5 h-3.5" /></>
                  : <><span>Next</span><ChevronRight className="w-3.5 h-3.5" /></>}
              </button>
            ) : (
              <button onClick={handleFinish} disabled={saving} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-emjsc-navy hover:bg-emjsc-red text-white rounded-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all disabled:opacity-50">
          ...
          {(currentStep === 'bio' || currentStep === 'password') && (
            <button onClick={currentStep === 'password' ? handleFinish : goNext} disabled={saving} className="w-full py-1.5 text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-colors disabled:opacity-50">
