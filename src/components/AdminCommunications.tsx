...
export function AdminCommunications({
  ...
  const startEditFeedback = (ann: any) => {
    setEditingId(ann.id);
    setEditPlayer(ann.targetPlayer || '');
    setEditContent(ann.content || '');
  };

  const saveEditFeedback = (ann: any) => {
      ...
      <div className="bg-slate-50 p-5 rounded-3xl border border-slate-100 space-y-4">
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <select value={feedbackPlayer} onChange={(e) => setFeedbackPlayer(e.target.value)} className="p-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-emjsc-navy outline-none focus:ring-1 focus:ring-emjsc-navy">
              <option value="">Select player…</option>
              {TEAM_SQUAD.map((p) => <option key={p.name} value={p.name}>{p.name}</option>)}
            </select>
            <div className="flex justify-end items-start">
              <button
                onClick={() => { if (!feedbackContent.trim() || !feedbackPlayer) return; onAddAnnouncement(feedbackContent.trim(), 'player_feedback', feedbackPlayer); setFeedbackContent(''); setFeedbackPlayer(''); }}
                className="bg-emerald-600 text-white text-[9px] font-black uppercase px-4 py-2.5 rounded-xl shadow-md active:scale-95 flex items-center gap-1.5"
              >
                <Plus className="w-3 h-3" />Add
              </button>
            </div>
          </div>
          <textarea value={feedbackContent} onChange={(e) => setFeedbackContent(e.target.value)} placeholder="Write feedback for this player…" className="w-full h-16 p-3 bg-white border border-slate-200 rounded-xl text-[11px] font-medium text-slate-700 outline-none focus:ring-1 focus:ring-emjsc-navy resize-none" />
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-100 border-b border-slate-200">
                  ...
                  <th className="px-3 py-2 text-[8px] font-black uppercase tracking-widest text-slate-400 w-16">Date</th>
                  ...
                  editingId === ann.id ? (
                    <tr key={ann.id} className="bg-emerald-50">
                      ...
                      <td className="px-2 py-2">
                        <div className="flex items-center gap-1">
                          <button onClick={() => saveEditFeedback(ann)} className="p-1 bg-emerald-600 text-white rounded-lg"><Check className="w-3 h-3" /></button>
                    ...
                    <tr key={ann.id} className="bg-white hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-2 py-2.5">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => startEditFeedback(ann)} className="p-1 text-emjsc-navy hover:bg-emjsc-navy/10 rounded-lg"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
